...
static double *tcol1 = NULL;
static double *tcol2 = NULL;
static double *tcol3 = NULL;
static double *temp1 = (double *) NULL;
...
ArnoldiROM(B, C, D, P, size, numinp, numout, q_orig,
  ...
  charge *chglist;
{
  // static double **H = NULL;   // Enrico, see header
  static double **V, **R1, **W, **X, **Z;
  int i, j, k, iter, bter;
  int s, q;
  ...
  if (H != NULL)
    viewprintf(stderr, "ArnoldiROM called more than once, you should fix memory \
to make this efficient\n");
 ...
 viewprintf(stdout, "Computing %d x %d matrix vector products for Reduced order model:\n",
         q, s);
  ...
  int numlin, numcol, block;
{
  int k, j, i;
  double normsq;
  // static double *tcol1 = NULL;  // Enrico, see header

  if (tcol1 == NULL)
    tcol1 = (double *) MattAlloc(numlin, sizeof(double));
  ...
  for (k = 0; k < numcol; k++) {
    ...
    for (i = 0; i < numlin; i++)
      tcol1[i] = Bmat[i][k];
    /* Pw = z */
    spSolve(P, tcol1, tcol1);
    /* get the P-norm of the k-th column (wk' * zk) */
    for (normsq = 0.0, i = 0; i < numlin; i++)
      normsq += (tcol1[i] * Bmat[i][k]);
    Rmat[k][k] = sqrt(normsq);
    /* normalize current vector, and copy tcol1 into Z */
    for (i = 0; i < numlin; i++) {
      Qmat[i][block * numcol + k] = Bmat[i][k] / Rmat[k][k];
      Z[i][block * numcol + k] = tcol1[i]/Rmat[k][k];
    }
    /* orthogonalize */
    for (j = k+1; j < numcol; j++) {
      /* compute and store cross-product */
  ...
  SYS *indsys;
{
  ...
  if (numinp != numout) {
    viewprintf(stderr,"Error: Inputs and outputs assumed to be coincident. No equiv cicuited dumped\n");
    return;
  }
  ...
  /* dump Dr; check to see if there is really a direct term */
  if (Dr != (double **)NULL) {
    viewprintf(stderr, "D matrix not supported! (but would just be a bunch of VCCS's)\n");
    return;
  }
...
{
  int i, j, realsize;
  //static double *temp1 = (double *) NULL;	// Enrico, see header
  int machine;
  ...
  if (temp1 == (double *) NULL) {
    realsize = MAX(size, numinp);
    realsize = MAX(realsize, numout);
    temp1 = (double *) DCALCORE(realsize, sizeof(double));	// Enrico, DCALCORE instead of calloc
  }															// to zero memory out (CALCORE does not zero)

  if (A != (double **)NULL) {
    for(j = 0; j < size; j++) {
      for(i = 0; i < size; i++)
        temp1[i] = A[i][j];
      savemat_mod(fp, machine+100, "Asys", size, size, 0, temp1,
                  (double *)NULL, j, size);
    }
  }
  if (B != (double **)NULL) {
    for(j = 0; j < numinp; j++) {
      for(i = 0; i < size; i++)
        temp1[i] = B[i][j];
      savemat_mod(fp, machine+100, "Bsys", size, numinp, 0, temp1, 
                  (double *)NULL, j, size);
    }
  }
  if (C != (double **)NULL) {
    for(j = 0; j < numout; j++) {
      for(i = 0; i < size; i++)
        temp1[i] = C[i][j];
      savemat_mod(fp, machine+100, "Csys", size, numout, 0, temp1, 
                  (double *)NULL, j, size);
    }
  }
  if (D != (double **)NULL) {
    for(j = 0; j < numinp; j++) {
      for(i = 0; i < numout; i++) 
        temp1[i] = D[i][j];
      savemat_mod(fp, machine+100, "Dsys", numout, numinp, 0, temp1, 
  ...
  SYS *indsys;
{
  ...
  if (err != 0) {
    viewprintf(stderr,"Couldn't create sparse matrix, err %d\n", err);
    ...
    for (i = 0; i < num_fils; i++) {
	  ...
	  for (mtrani=Mtrans[filnum]; mtrani != NULL; mtrani = mtrani->mnext) {
            if (mtrani->filindex + 1 > indsys->num_mesh ||
              ...
              viewprintf(stderr, "Indexing into matrix MRMt out of bounds\n");
              FHExit(FH_GENERIC_ERROR);
            }
  ...
  char *MRMt;
{
  ...
  tcol2 = (double *) MattAlloc(rows, sizeof(double));
  temp2 = (double *) MattAlloc(nmesh, sizeof(double));

  /* this does L*(Mt)_i where (Mt)_i is a single column (row) of Mt (M) and
     saves it in the temp space, tcol2 */
  for (mesh = 0; mesh < num_mesh; mesh++) {
     ...
     *      -(M*R*Mr) X = Vs
     */
    spSolve(MRMt, temp2, temp2);
    for (mesh2 = 0; mesh2 < num_mesh; mesh2++) {
      MRMtinvMLMt[mesh2][mesh] = -temp2[mesh2];
    }
  }
  ...
  int rowcol, rownum, colnum;
{
  ...
  if (localfp == NULL) {
   viewprintf(stderr, "Couldn't open DUMPS file\n");
    FHExit(FH_GENERIC_ERROR);
  }
...
SYS *indsys;
{
  ...
  if (tcol3 == NULL)
    tcol3 = (double *)MattAlloc(rows, sizeof(double));

  /* this does L*(Mt)_i where (Mt)_i is a single column (row) of Mt (M) and
     saves it in the temp space, tcol3 */
  for(mesh = 0; mesh < num_mesh; mesh++) {
    for(j = 0; j< nfils; j++)
      tcol3[j] = 0;
    /* note, this next set of nested loops could be reversed, but I think
    ...
    for(melem = Mlist[mesh]; melem != NULL; melem = melem->mnext)
      for(j = 0; j < nfils; j++)
	tcol3[j] += L[j][melem->filindex]*melem->sign;
    for(mesh2 = 0; mesh2 < num_mesh; mesh2++) {
      tempsum = 0;
      for(melem2 = Mlist[mesh2]; melem2 != NULL; melem2 = melem2->mnext)
	tempsum += melem2->sign*tcol3[melem2->filindex];
