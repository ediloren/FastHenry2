...
int i, j;
CX **diagBlock;
  if(startrows[0] != 0) {viewprintf(stderr,"startrow not zero\n"); FHExit(FH_NORMAL_END);}
  if(startrows[numblocks] != size) {viewprintf(stderr,"startrow not size\n"); FHExit(FH_NORMAL_END);}
  ...
  int cond;   /* conductor number for saving residual data */
{
  ...
  if((size > lastsize) || (maxiters > lastmax)) {
    ...
    if (y == NULL) {viewprintf(stderr,"no space \n"); FHExit(FH_GENERIC_ERROR); }
  ...
  if (indsys->opts->debug == ON)
   viewprintf(stderr,"zeta=%lg (initial rnorm)\n",zeta);
...
/*    cx_mul(g[k], c[k], hi);  need conjugate */
    ...
    if (indsys->opts->debug == ON) {
     viewprintf(stderr,"%d rnorm=%g real_part=%lg imag_part=%lg\n", 
	     k+1,rnorm, r_real, r_imag);
      fflush(stdout);
    }
    else {
      viewprintf(stdout,"%d ",k+1);
      fflush(stdout);
   ...
   viewprintf(stderr,"check norm =%g\n", blahnorm); 
#endif
  ...
  if (k == maxiters - 1) 
   viewprintf(stderr,"Warning: Convergence not reached. Stopping iteration...\n");
  if (indsys->opts->debug == ON)
   viewprintf(stdout,"Total GMRES iters = %d\n", k+1);
  else
   viewprintf(stdout,"\n");
...
int size;
{
  ...
  for(i=0; i < size; i++) {
   viewprintf(stdout,"\n");
    for(j=0; j < size; j++) {
     viewprintf(stdout,"r=%g i=%g ", A[i][j].real, A[i][j].imag);
    }
  }
 viewprintf(stdout,"\n");
}
