...
static double one_6_1 = 1.0/6.0;
static double one_6_2 = 1.0/6.0;
...
*/
  ...
  // z_j is the unit vector along filament j length
  ...
  */
  ...
  if ( fabs(fabs(l3 - l3_1) - l2)/l2 > EPS) {
    viewprintf(stderr, "Huh?  filament not expected length\n");
  ...
  if (a_deg && b_deg && c_deg && d_deg) {
    ...
    viewprintf(stderr,"Internal Error: Bad Degenerate filament a/l1<tol\n");
    viewprintf(stderr,"Using fourfil() instead...\n");
    totalM = fourfil(fil_j, fil_m)/(sign*MUOVER4PI);
  }
  else
...
#ifndef SGI
  if (_isnan(totalM)) /* Enrico, added _ before function call (Windows func is _isnan() ) */
    viewprintf(stderr, "Exact_mutual returned NaN! filaments %d, %d\n",fil_j->filnumber, fil_m->filnumber);
#endif
...
double x,y,z, ref_len;
{
  ...
  if (num_nearzero < 1)
    retval -= one_6_1*(tan_term(x,y,z,zsq,len) + tan_term(x,z,y,ysq,len) 
...
*/
  ...
  if (i != num_dims) {
    viewprintf(stderr, "bad number for num_dims %d\n",num_dims);
    FHExit(FH_GENERIC_ERROR);
  }
}
/* this fills the vector dims with the dimension information for fil_j
...
AllocInfo *allocptr;
{
  ...
  if (allocptr->elems_left > 0) {
    ...
    elem = (AllocList *) DMALCORE(sizeof(AllocList));	// Enrico
    elem->next = allocptr->head;
    allocptr->head = elem;
    elem->ptr = allocptr->next_elem = 
                        (char *) DMALCORE(allocptr->size*blocksize);	// Enrico
    allocptr->elems_left = blocksize - 1;
    if (allocptr->next_elem == NULL) {
      viewprintf(stderr, "Out of memory in AllocAnEntry. size = %d, block = %d\n",
...
double x,y,z, ref_len;
{
  ...
  retval = -one_6_2*len*(xsq - 2*ysq + zsq);
...
double x,y,z, ref_len;
{
  ...
  if (!(nzzsq && nzysq))
    retval += (0.5*zsq - one_6_3*ysq)*y*log(x + len);
  if (!(nzzsq && nzxsq))
    retval += (0.5*zsq - one_6_3*xsq)*x*log(y + len);
  if (!(nzzsq || nzysq || nzxsq))
    retval += x*y*z*log(z + len); 

/*  retval += (0.5*zsq - one_6_3*ysq)*y*log(x + len)
                + (0.5*zsq - one_6_3*xsq)*x*log(y + len)
		  + x*y*z*log(z + len); 
*/
  ...
  if (num_nearzero < 1)
    retval -= zsq*z*one_6_3*tan_tape(x,y,z,len) 
