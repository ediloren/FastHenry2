...
FILAMENT *fil_j, *fil_m;
{
  ...
  if (MAX(rj,rm)*100 < dist) { 
    ...
    if (!_finite(totalM))  /* Enrico, added _ before function call (Windows func is _finite() ) */
    ...
    */
	...
	sum1 += Gweight[nfilsj][i]*sum2;
      ...
      viewprintf(stdout,"gss: %14.8le ", sum1/4.0);
     ...
     FILAMENT *fil1, *fil2;
{
  ...
  viewprintf(stderr,"Severe warning: mutual inductance = infinity for two filaments:\n");

  fil = fil1;
  viewprintf(stderr,"  fil 1: from %lg, %lg, %lg to %lg, %lg, %lg  width:%lg height: %lg\n",
	  fil->x[0],fil->y[0],fil->z[0],fil->x[1],fil->y[1],fil->z[1],
	  fil->width, fil->height);
  fil = fil2;
  viewprintf(stderr,"  fil 2: from %lg, %lg, %lg to %lg, %lg, %lg  width:%lg height: %lg\n",
	  fil->x[0],fil->y[0],fil->z[0],fil->x[1],fil->y[1],fil->z[1],
	  fil->width, fil->height);
  viewprintf(stderr,"Probably because there are overlapping but non-orthogonal segments in the input\n");
}
...
*/
  ...
  if (m/(R1 + R2) > 1.0-EPS) {
    ...
    viewprintf(stderr,"  results will still be accurate\n");
	return M;
  }
  if (m/(R3 + R4) > 1.0-EPS) {
...
double dist;
{
  ...
  if (deg_j == brick && deg_m == brick) {
    ...
    /* viewprintf(stderr,"Nondegenerate: fil %d to fil %d: %13.6lg\n",
           fil_j->filnumber,
	   fil_m->filnumber, 
    ...
    /* viewprintf(stderr,"  degenerate: fil %d to fil %d: %13.6lg\n",
           fil_j->filnumber,
	   fil_m->filnumber, 
