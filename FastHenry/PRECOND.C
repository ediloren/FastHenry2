...
static int *filcount = NULL;  
static int *filcount2 = NULL; 
static int *maxfilcount = NULL; 
static int *indx = NULL;      
static int *meshnum = NULL;
...
/*    if (indsys->precond_type == LOC && ) {*/
	...
	if (filcount2[i] > 0) {
      ...
      viewprintf(stderr,"What kind of precondtioner?\n");
      FHExit(FH_GENERIC_ERROR);
    }
    ...
    if (counter != meshsize) {
      viewprintf(stderr, "Hey, counter should equal meshsize\n");
      FHExit(FH_GENERIC_ERROR);
    }
	...
	for(mtranj=Mtrans[fillist[j]]; mtranj != NULL; mtranj = mtranj->mnext)
	  {
	      ...
	      for(mtrani=Mtrans[fillist[i]]; mtrani!=NULL;mtrani=mtrani->mnext)
		{
         ...
         viewprintf(stderr,"Possible Bug:  self term in preconditioner == 0!\n");
...
//	fp = fopen("chkinv.mat","w");
	...
	viewprintf(stdout, "Inverting a %d x %d matrix\n",meshsize,meshsize);
	...
	if (filcount[ realmrow ] > maxfilcount[ realmrow ]) {
	  ...
	  for(j = 0, pre = Precond[realmrow]; j < meshsize; j++) {
	    if (pre == NULL) {
	      ...
	      pre->next = NULL;
	      if (prelast == NULL) {
		viewprintf(stderr, "Hey, prelast is null!\n");
		FHExit(FH_GENERIC_ERROR);
	      }
	      prelast->next = pre;
	    }
...
/*  bigmeshPre(sys, indsys, w);  */
  ...
  if (indsys->precond_type == LOC) {
	...
	/* make the identity */
	if (indsys->opts->debug == ON)
	 viewprintf(stderr, "mesh %d partial everywhere\n",i);
	...
	for(melem = Mlist[i]; melem != NULL; melem = melem->mnext) {
	  ...
	  if (j == nc->directnumeles[0]) {
	    viewprintf(stderr,"Hey, why isn't the charge in the cube?\n");
	    FHExit(FH_GENERIC_ERROR);
	  }
	...
	if (indsys->opts->debug == ON)
	  viewprintf(stdout, "Sum of self terms: %lg +i*%lg\n",tempsum.real, tempsum.imag);
	
#else
	viewprintf(stderr, "Hey, mesh %d is never included in any cube??\n",i);
	FHExit(FH_GENERIC_ERROR);
#endif
      }
  }
    ...
    for(i = 0; i < num_mesh; i++)
      if (maxfilcount[i] == 0) {
	viewprintf(stderr, "Internal Err: mesh %d not used in Preconditioner\n"
		,i);
	count++;
      }
    if (count != 0)
  ...
  if (indsys->opts->debug == ON)
    viewprintf(stdout, "Actual PrecCost: %lg, aver_mat:%lg\n",
  ...
  int different, sign;
	...
	for(j = i + 1; j < meshsize; j++) {
	  if (is_dup[j].sign == 0) {
		...
		is_dup[j].sign = sign;
		is_dup[j].dup = i;
		viewprintf(stderr,"Duplicate mesh marked\n");
	      }
	  } /* endif */
	} /*for j*/
      } /* endif */
...
char *suffix;
{
 ...
 viewprintf(stdout, "Dumping Preconditioner...\n");
...
//  fp = fopen(outfname,"w");
  ...
  if (fp == NULL) {
    viewprintf(stderr,"Couldn't open Pre\n");
    FHExit(FH_GENERIC_ERROR);
  }
