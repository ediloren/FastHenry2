...
int mid,o1,o2;
{
  ...
  if (fabs(value - equation)/(xpart*xpart+ypart*ypart+zpart*zpart+1e-200) 
   ...
   viewprintf(stderr, "Possible internal error: something wrong with coordinates %lg %lg %lg\n",x,y,z);
    return 0;
  }
}
...
int flag;
{
  ...
  if(flag == 0){                           /* need to find the x_coordinate */
   ...
   viewprintf(stderr, "flag does not match coordinates \n");
...
GROUNDPLANE *plane;
{
  ...
  if((nodein->gp != plane) || (nodeout->gp != plane)){
   viewprintf(stderr, "nodes not in the groundplane !! \n");
    FHExit(FH_GENERIC_ERROR);
  }
...
/*    printf("find_nearest_gpnode worked!\n"); */
   ...
   viewprintf(stderr, "find_nearest_gpnode failed!!!\n");
  ...
  if(deco1 > 0){
    if(counter1 != e0){
     viewprintf(stderr, "something wrong with .external stuff...\n");
      FHExit(FH_GENERIC_ERROR);
    }
  } else {
    if((counter1 + 1) != e0){
...
GROUNDPLANE *plane;
{
  ...
  if((nodein->gp != plane) || (nodeout->gp != plane)){
    viewprintf(stderr, "path_though_gp: nodes not in the groundplane !! \n");
    FHExit(FH_GENERIC_ERROR);
  }
  ...
  if (path == NULL) {
    viewprintf(stderr, "Error: Couldn't find a path from node %s to %s.\n",
...
SYS *indsys;
{
  ...
  if ( (fp = fopen(outfname,"wb")) == NULL) {		// Enrico
    viewprintf(stderr, "No open meshes.mat\n");
    FHExit(FH_GENERIC_ERROR);
  }
