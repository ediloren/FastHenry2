...
int is_initial;    /* is this the initial refinement call */
{
  ...
  for(seg = indsys->segment; seg != NULL; seg = seg->next) {
	...
	if (seg->type == NORMAL) 
	  ...
	  viewprintf(stderr, "DivideSegs: Warning: tried to divide an indivisable segment.\n");
	  viewprintf(stderr, "  Segment length: %lf,  maximum allowed length: %lf",
		  seg->length, length/DIVFACT);
	  viewprintf(stderr, "  The segment is probably part of a ground plane.\n");
	  viewprintf(stderr, "  If so, decrease the partitioning level by 1 or refine the ground plane\n");
	  warned = 1;
	}
      }
      else if (!warn_refine) {
	...
	if (indsys->opts->mat_vect_prod == MULTIPOLE)
	  viewprintf(stderr, "Warning: couldn't refine segments as needed because auto_refine == OFF.\n This will decrease multipole accuracy.\n");
      }
...
SYS *indsys;
{
  ...
  if (nodelast->type != NORMAL) {
    viewprintf(stderr,"Internal bug.  nodelast->type != NORMAL\n");
