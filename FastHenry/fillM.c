...
SYS *indsys;
{
  ...
  if (mesh > indsys->extra_meshes) {
    viewprintf(stderr, "Internal Error: Bad estimate for extra_meshes\n");
    viewprintf(stderr, "   One solution is to change FILS_PER_MESH to 1\n");
  ...
  if (mesh <= indsys->num_mesh) 
    ...
    viewprintf(stderr, "uh oh, mesh > num_mesh\n");
    FHExit(FH_GENERIC_ERROR); 
  }
}
...
SYS *indsys;
{
  ...
  for(selem = path, i = 0; selem != NULL; selem = selem->next, i++) {
    ...
    if (getrealnode(plusnode) == getrealnode(node0))
      ...
      viewprintf(stderr, "make_mesh_from_path: segments don't connect at node %s\n", plusnode->name);
      FHExit(FH_GENERIC_ERROR);
    }
	...
	while( is_next_seg_in_gp(selem, plusnode) == TRUE) {
	  ...
	  if (indsys->opts->debug == ON)
	   viewprintf(stderr, "Fixing extra long gp mesh in gp %s, mesh %d.\n",
		   node0->gp->name, mesh);
	}
	if (sign == 1) {
	...
	while(temppath != NULL) {
	  ...
	  if (is_nonuni_gp(node0->gp)) {
	    ...
	    if (plus2->gp_node == seg->node[0]->gp_node) {
	     ...
	     viewprintf(stderr, "Hey, path_through_gp made nonconnected path!\n");
	     FHExit(FH_GENERIC_ERROR);
	    }
	  }
	  else {
...
/*	  free(telem); */
	...
	viewprintf(stderr, "make_mesh_from_path: unknown pseudo_seg %d\n",
		pseg->type);
	FHExit(FH_GENERIC_ERROR);
      }
    }
...
seg_ptr seg;
{
  viewprintf(stderr, "%s: bad seg type: %d\n",name, seg.type);
  FHExit(FH_GENERIC_ERROR);
}
...
SYS *indsys;
{
  ...
  if (port == NULL) {
    viewprintf(stderr, "Hey, supposed external segment isn't in list\n");
    FHExit(FH_GENERIC_ERROR);
  }
...
int mstart;
{
  ...
  if(counter > plane->numesh){
   viewprintf(stderr, "something wrong with meshes, numesh != counter \n");
    FHExit(FH_GENERIC_ERROR);
  }
...
int column;
{
  ...
  if (Ib == NULL) {
     ...
     fname = MALCORE(100*sizeof(char));		// Enrico, MALCORE instead of malloc
     tempstr = MALCORE(100*sizeof(char));	// Enrico, MALCORE instead of malloc
   }
 ...
 viewprintf(stdout, "saving to Jreal%s%d_%d.mat, Jimag%s%d_%d.mat, Jmag%s%d_%d.mat\n",
...
/*  fprintf(fpmag, "$ DATA=VECTOR\n");*/
 ...
 viewprintf(stdout, "saving to file Grid%s%d_%d...\n",indsys->opts->suffix,
	 column+1,freq_num);
	...
	for(j = 0; j < dir1; j++) {
	  ...
	  if(j != dir1 - 1 && p->segs1[j][i] != NULL) {
	    ...
	    if (p->segs1[j][i]->node[0] != p->pnodes[j][i]) {
	     viewprintf(stderr, "You goofed 1\n");
	    }
	  }
	  else 
	    out1[i][j] = CXZERO;
	  ...
	  if (i != dir2 - 1 && p->segs2[j][i] != NULL) {
	    ...
	    if (p->segs2[j][i]->node[0] != p->pnodes[j][i]) {
	     viewprintf(stderr, "You goofed 2\n");
	    }
	  }
	  else
	    out2[i][j] = CXZERO;
     ...
     viewprintf(stdout, "saving grid1%s...\n",p->name);
      strcpy(fname, "grid1");
     ...
     viewprintf(stdout, "saving grid2%s...\n",p->name);
      strcpy(fname, "grid2");
      sprintf(tempstr, "%s",p->name);
      strcat(fname,tempstr);
...
/*	add_to_external(pseg, mesh, sign, indsys); */
  ...
  if (extern_count > 0) {
    if (extern_count == 1)
      ...
      viewprintf(stderr,"Internal Err: More than one voltage source in a mesh\n");
      FHExit(FH_GENERIC_ERROR);
    }
  }
  ...
  if (mesh == 0) {
    viewprintf(stderr, "make_many_meshes: No meshes made!\n");
    FHExit(FH_GENERIC_ERROR);
  }
...
int total_meshes;
{
  ...
  if (counter != big_meshes) {
    viewprintf(stderr, "Error getting representative meshes\n");
    FHExit(FH_GENERIC_ERROR);
  }
...
MELEMENT **Mlist;
{
  ...
  for(i = 0; i < undone; i++) {
    ...
    if (m_info[j].other_mesh == 0) {
      viewprintf(stderr, "Huh?  mesh isn't identical to itself?\n");
      FHExit(FH_GENERIC_ERROR);
    }
  }
}
...
int constraining_mesh;
{
  ...
  if (m_begin->num_meshes != 1) {
    if (other_mesh == -1) {
      viewprintf(stderr, "Huh?  other_mesh == -1?\n");
      FHExit(FH_GENERIC_ERROR);
    }
   for(i = m_begin->first; i < m_begin->first + m_begin->num_meshes; i++) {
