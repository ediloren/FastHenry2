     ...
     GROUNDPLANE *plane;
{
  ...
  if (in == out) {
    ...
    /*
    viewprintf(stderr, "Warning: user gp node at (%lg, %lg, %lg) (meters) and node at (%lg, %lg, %lg)\n", nodein->x, nodein->y, nodein->z, nodeout->x, 
	    nodeout->y, nodeout->z);
    viewprintf(stderr, "in Nonuniform gp %s correspond to the same node\n",
	    gp->grndp->name);
    return NULL;  
    */
  }
     ...
     NPATH *node_list;
{
  ...
  if (nonuni_node == NULL) {
    viewprintf(stderr,"Error: Node for plane %s requested at %lg %lg %lg doesn't appear to be within the plane\n",
	    gp->grndp->name, x, y, z);
