...
int size;
{
  ...
  temp = DMALCORE(size);		// Enrico, MALCORE instead of malloc

  if (temp == NULL) {
    viewprintf(stderr, "Gmalloc: out of space. %d bytes needed\n", size);
  }
...
/*
  if (realnode->equiv != realnode) {
    viewprintf(stderr, "Hey, node->equiv chain more than one node long!\n");
...
seg_ptr tseg;
{
  ...
  if (tseg.type == NORMAL) {
    ...
    if (seg->node[0] == node)
      ...
      if (node0 == node1) {
	viewprintf(stderr, "getothernode: node1==node0, this cause infinite loop\n");
	FHExit(FH_GENERIC_ERROR);
      }
      if (node0 == realnode)
	...
	viewprintf(stderr, "Hey, node %s isn't on seg %s\n",node->name, seg->name);
	FHExit(FH_GENERIC_ERROR);
      }
    }
  }
    ...
    if (pseg->node[0] == node)
      ...
      realnode = getrealnode(node);
      if (node0 == node1) {
	viewprintf(stderr, "getothernode: node1==node0, this would cause an infinite loop\n");
	FHExit(FH_GENERIC_ERROR);
      }
      if (node0 == realnode)
	...
	viewprintf(stderr, "Hey, node %s isn't on pseudo_seg\n",node->name);
	FHExit(FH_GENERIC_ERROR);
      }
    }
  }
...
SYS *indsys;
{
  ...
  while(notblankline(line)) {
    if (sscanf(line,"%s%n",name1,&skip) != 1) {
     viewprintf(stderr,"equivnodes: sscanf error on equiv line\n");
      return 1;
    }
    line += skip;
  ...
  if (nlist == NULL) {
    viewprintf(stderr, "equivnodes: No real nodes in equiv statement\n");
    FHExit(FH_GENERIC_ERROR);
  }
...
NODES *node;
{
  ...
  if (name[0] != 'n') {
    viewprintf(stderr, "create_pn: Invalid node name %s. First letter should be 'n'\n",
	    name);
    FHExit(FH_GENERIC_ERROR);
  }
...
NODES *orignode, *realnode;
{
  ...
  if(is_real_node(realnode) == 0 ) {
    viewprintf(stderr, "make_equiv: Internal error: Can't equiv to a nonreal node\n");
    FHExit(FH_GENERIC_ERROR);
  }

/* can equiv before this is connected to anything, so this isn't an error
  if (realnode->connected_segs == NULL) {
    viewprintf(stderr, "Internal error: make_equiv: connected_segs == NULL\n");
    FHExit(FH_GENERIC_ERROR);
  }
*/

  if (node == realnode) {
    viewprintf(stderr,"Warning: Trying to equiv nodes that are already equiv'ed.\n");
    viewprintf(stderr,"  Nodes:  %s and %s\n",orignode->name, realnode->name);
    if (is_gp_node(orignode) || is_gp_node(realnode))
      viewprintf(stderr, "  Maybe ground plane is not fine enough (two nodes in same place)\n");
    return;
  }
...
NODES *node;
{
  ...
  if (node != NULL && node != getrealnode(node)) {
    viewprintf(stderr, "Internal err: External node not a real node. node: %s\n",
	    node->name);
    FHExit(FH_GENERIC_ERROR);
  }
...
PSEUDO_SEG *pseudo_seg;
{
  ...
  if (seg != NULL)
    ...
    viewprintf(stderr, "remove_from_connected_segs: Either seg or pseudo_seg must by NULL\n");
    FHExit(FH_GENERIC_ERROR);
  }
  ...
  if (segelem == NULL) {
    viewprintf(stderr, "remove_from_  : No segs connected to node %s\n", realnode->name);
    FHExit(FH_GENERIC_ERROR);
  }
  ...
  if (segelem->seg.segp == vptr && segelem->original == node) {
    ...
    if (segelem->next == NULL) {
      viewprintf(stderr, "remove_from_  : Couldn't remove seg from node %s\n",
	      realnode->name);
      FHExit(FH_GENERIC_ERROR);
    }
    else {
  ...
  while(segelem != NULL && 
    ...
    segelem = segelem->next;
  if (segelem != NULL) {
    viewprintf(stderr, "remove_from_ :What?  This seg is in node %s twice\n",
	    realnode->name);
    FHExit(FH_GENERIC_ERROR);
  }
}
...
int *i, *j;
{
  // static int mid = 1;  // Enrico, see header
  static int o1 = 0, o2 = 2;     
  ...
  *j = count2;
  ...
  if ((distance - gp->unitdiag/2.0)/gp->unitdiag/2.0 > 1e-3 ) {
    viewprintf(stderr, "Warning: (%lg, %lg, %lg) is more than half a cell away from plane %s\n",
	    x, y, z, gp->name);
    viewprintf(stderr, "  %lg > %lg.  Is the point really on the plane?\n",
	    distance, gp->unitdiag/2.0);

    if (dotp(gp->ux1,gp->uy1,gp->uz1,gp->ux2,gp->uy2,gp->uz2) > 1e-13) {
      viewprintf(stderr,"find_nearest_gpnode: Edges of plane not perpendicular.\n");
      viewprintf(stderr," This is not yet supported in this function \n");
    }
  }
...
char type;
{
  ...
  if (type == GPTYPE) {
    if (node1->gp != node2->gp) {
      viewprintf(stderr,"Internal Err: gp pseudo_seg nodes not in same plane!\n");
      FHExit(FH_GENERIC_ERROR);
    }
    ...
    viewprintf(stderr, "Internal Error: Unknown type of pseudo_seg\n");
    FHExit(FH_GENERIC_ERROR);
  }
#endif 
...
TREE *tree;     /* tree that this loop will be contained within */
{
  ...
  if (node_s != node_l) { /* is this not a self loop? */
...
#endif
    if (count < 0) {
      viewprintf(stderr, "make_loop:  strange loop. count < 0?\n");
      FHExit(FH_GENERIC_ERROR);
    }
    while(count > 0) {
   ...
   junk
	...
	viewprintf(stderr, "Internal Error in make_loop: Hey, these don't make a loop!\n");
	FHExit(FH_GENERIC_ERROR);
      }
#endif
    pre_seg = node_s->pred;
    path = add_seg_to_list(pre_seg, path);
    if (getrealnode(getothernode(node_s, pre_seg)) != pre_node) {
      viewprintf(stderr, "make_loop: Hey, these don't make a loop!\n");
      FHExit(FH_GENERIC_ERROR);
    }
#if 1==0
  junk
	...
	viewprintf(stderr, "Internal Error in make_loop: Hey, these don't make a loop (extern)!\n");
	FHExit(FH_GENERIC_ERROR);
      }
    }
#endif 
...
int fils_per_mesh;
{
  ...
  while(trees != NULL) {
	...
	if (is_normal_seg(one_seg->seg))
	  ...
	  viewprintf(stderr,"Internal Err: bad seg type in estimate_ext...\n");
	  FHExit(FH_GENERIC_ERROR);
	}
      }
...
TREE *atree;
{
  ...
  if (counter > 1) {
   viewprintf(stderr,"Warning: Multiple boundaries found around one hole region\n");
   viewprintf(stderr,"  possibly due to an isolated or nearly isolated region of conductor.\n");
   viewprintf(stderr,"  This may lead to no unique solution.\n");
  }
