	...
	for(j = size - 1; j >= 0; j--) {
	  ...
	  for(k = nc->evalnumeles[i] - 1; k >= 0; k--) {
	    ...
	    if (sys->DirectEval == TRUE)
	      ...
	      {printf("huh?"); FHExit(FH_GENERIC_ERROR);}
#endif
	  }
	}
      }
...
printops()
{
  viewprintf(stdout,"Number of Direct Multi-Adds = %d\n", directops);
  viewprintf(stdout,"Number of Upward Pass Multi-Adds = %d\n", upops);
  viewprintf(stdout,"Number of Downward Pass Multi-Adds = %d\n", downops);
  viewprintf(stdout,"Number of Evaluation Pass Multi-Adds = %d\n", evalops);
  viewprintf(stdout,"Number of Evaluation Pass Direct Multi-Adds = %d\n", evaldops);
  viewprintf(stdout,"Number of Evaluation Pass Other Multi-Adds = %d\n", evalmops);
  viewprintf(stdout,"Total Number of Multi-Adds = %d\n", directops+upops+downops+evalops);
}
#endif

// init globalstatic vars
