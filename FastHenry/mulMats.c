...
static int big_mat_size = 0;

// Enrico, added prototype 
void find_flux_density_row(double **to_mat, double **from_mat, 
						   int eval_row, int n_chg, int n_eval, int row_offset, int col_offset,
						   charge **eval_panels, charge **chg_panels, 
						   int *eval_is_dummy, int *chg_is_dummy);
...
/* Now place in the matrices. */
  for(nextc=sys->directlist; nextc != NULL && bFHContinue == TRUE; nextc = nextc->dnext) {  // Enrico
...
#if JACDBG == ON
  viewprintf(stdout,"max direct size =%d\n", maxsize);
#endif
...
#if DPCOMP == ON
	viewprintf(stdout, "Source mat, nc to nc\n");
...
#if DPCOMP == ON
	    viewprintf(stdout, "Source mat, nnbr to nc\n");
...
#if DPCOMP == ON
		viewprintf(stdout, "Source mat, nnnbr to nnbr\n");
...
#if DPDDIF == ON
  viewprintf(stdout, "Center row, factor = %g\n", factor);
#endif
...
#if DPDDIF == ON
    viewprintf(stdout, " %.16e", from_mat[eval_row][j]);
#endif				/* #if DPDDIF == ON */
  }
#endif				/* #if NUMDPT == 3 */
...
#endif
#if DPDDIF == ON
      viewprintf(stdout, " %.16e (%d)", from_mat[dindex][j],chg_panels[j]->index);
#endif
    }
  }
...
#endif
#if DPDDIF == ON
	viewprintf(stdout, " %.16e (%d)",
		calcp(chg_panels[j], dp, NULL),
		chg_panels[j]->index);
      }
      else {
	viewprintf(stdout, " dummy");
#endif
      }
    }
  }
...
#else
    viewprintf(stderr, "olmulMatPrecond: building neg. dummy row\n");
#endif
    for(j = n_chg - 1; j >= 0; j--) {
#if SKIPQD == ON
      if(chg_panels[j]->index == eval_panels[eval_row]->index) continue;
...
{
	cube *nextc, *kid;
	int i, j, numterms, depth, order;
	double **multimats[8];
	char ismultimatsnull[8];
...
// Handle the lowest level cubes first (set up Q2M's).
  ...
  if(sys->locallist[sys->depth] == NULL
    ...
    viewprintf(stdout, "No expansions at level %d (lowest)\n", sys->depth);
    //if(sys->depth < 3) 
	//viewprintf(stdout, " (Warning: no multipole acceleration)\n");
  }
  else if(sys->locallist[sys->depth] == NULL) {
    viewprintf(stdout, "No local expansions at level %d (lowest)\n", sys->depth);
  }
  else if(sys->multilist[sys->depth] == NULL) {
    viewprintf(stdout, "No multipole expansions at level %d (lowest)\n", 
	    sys->depth); 
    //if(sys->depth < 3) 
	//viewprintf(stdout, " (Warning: no multipole acceleration)\n");
  }
...
// no multipoles over root cube or its kids (would not be used if made) 
  ...
  for(; depth > 1; depth--) {
    ...
    if(sys->locallist[depth] == NULL && sys->multilist[depth] == NULL) {
      viewprintf(stdout, "No expansions at level %d\n", depth);
      //if(depth < 3) viewprintf(stdout, " (Warning: no multipole acceleration)\n");
      //else viewprintf(stdout, "\n");
    }
    else if(sys->locallist[depth] == NULL) {
      viewprintf(stdout, "No local expansions at level %d\n", depth);
    }
    else if(sys->multilist[depth] == NULL) {
      viewprintf(stdout, "No multipole expansions at level %d\n", depth); 
      //if(depth < 3) viewprintf(stdout, " (Warning: no multipole acceleration)\n");
      //else viewprintf(stdout, "\n");
    }
    ...
    for(i=0; i < 8; i++) {
		multimats[i] = NULL;
		// Enrico, see below usage of ismultimatsnull[]
		ismultimatsnull[i] = TRUE;
	}
#endif
      ...
      for(i=0, j=0; j < nextc->numkids; j++) {
	    if((kid = nextc->kids[j]) != NULL) { // NULL => empty kid cube 
	      if(kid->mul_exact == FALSE) { // if kid has a multi 
			...
			// Enrico, warning bug fix: the original code had problems with optimizations
			// if check was done directly on multimats[j]: the VisualC++
			// compiler generated INTERNAL ERROR C1001;
			// so check has been shifted to a BOOL variable
	        ...
	        if(ismultimatsnull[j] == TRUE) { // Build the needed matrix only once. 
			  ismultimatsnull[j] = FALSE;
	          multimats[j] = mulMulti2Multi(kid->x, kid->y, kid->z, nextc->x, 
					    nextc->y, nextc->z, order);
			}
	        ...
	        nextc->upmats[i] = mulQ2Multi(kid->chgs, kid->nbr_is_dummy[0],
					  kid->upnumeles[0], nextc->x, nextc->y, nextc->z, order);
...
ssystem *sys;
{
  // Enrico, removed 'k' and 'vects' because not used
  ...
  for(nc = sys->directlist; nc != NULL && bFHContinue == TRUE; nc = nc->dnext) {  // Enrico
