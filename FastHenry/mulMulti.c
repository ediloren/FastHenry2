...
{
  if(m > n) {
    viewprintf(stderr, "index: m = %d > n = %d\n", m, n);
    FHExit(FH_NORMAL_END);
  }
  if(n < 0 || m < 0) {
    viewprintf(stderr, "index: n = %d or m = %d negative\n", n, m);
    FHExit(FH_NORMAL_END);
  }
  return(m + (n*(n+1))/2);
}
...
{
  if(m > n) {
    viewprintf(stderr, "sindex: m = %d > n = %d\n", m, n);
    FHExit(FH_NORMAL_END);
  }
  if(n < 0 || m < 0) {
    viewprintf(stderr, "sindex: n = %d or m = %d negative\n", n, m);
    FHExit(FH_NORMAL_END);
  }
  if(m == 0) {
    viewprintf(stderr, "sindex: attempt to index M%d^0\n", n);
    FHExit(FH_NORMAL_END);
  }
...
#if DISFAF == ON
  viewprintf(stdout, "FACTORIAL FACTOR ARRAY:\n");
...
#if DALQ2M == ON
  viewprintf(stdout,"    AFTER ADDITION OF RHO^N FACTORS\n");
  dumpMat(mat, terms, numchgs);
#endif
...
#if DALQ2M == ON
  viewprintf(stdout,"    AFTER COPYING SINE (LOWER) HALF\n");
  dumpMat(mat, terms, numchgs);
#endif
