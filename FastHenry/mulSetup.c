...
int pseudo_lev;
{
  ...
  if (length0 < maxTileLength) {
    if (indsys->opts->debug == ON) {
      viewprintf(stderr, "length of cube smaller than max fil length. %lg<%lg\n",
	      length0,maxTileLength);
      viewprintf(stderr, "No big deal.  It'll get broken later\n");
    }
    /*
  ...
  viewprintf(stdout, "Total number of filaments before multipole refine: %d\n",
	  totalq);
  ...
  if(flag == ON) {		/* set depth of partitions automatically */
	...
	if(cubes[i] == NULL) {
	  viewprintf(stderr, "placeq: %d levels set up\n", i-1);
	  FHExit(FH_NORMAL_END);
	}
	for(j=0; j < side; j++) {
	...
	for(k = 0; k < side; k++) {
      ...
      if (indsys->opts->debug == ON) {
	viewprintf(stdout, "Level %d, %g%% exact\n", i,exact_ratio*100.0);
	viewprintf(stdout, "Preconditioner Cost Estimate: %lg,  Average Matrix: %lg\n", Precond_Cost, aver_mat_size);
	viewprintf(stdout, "totalq: %d  #cubes: %d  Direct_Cost: %lg\n",totalq2,
...
*/
  ...
  viewprintf(stdout, "Total number of filaments after multipole refine: %d\n",
	  totalq);
  ...
  for(nextq = charges; nextq != NULL; nextq = nextq->next) {
    ...
    if(tilelength(nextq) > length) {
      ...
      viewprintf(stderr,"  cube length = %g panel length = %g\n", 
	      length, tilelength(nextq));
      /* disfchg(nextq); */
    }
#endif
...
#if 1== 0
	viewprintf(stderr, "placeq: Warning, removing identical");
	if(compq->shape == 3) viewprintf(stderr, " triangular");
	else if(compq->shape == 4) viewprintf(stderr, " quadrilateral");
	else viewprintf(stderr, " illegal-shape");
	viewprintf(stderr, " panel\n  rmved ctr = (%g %g %g) seg = `%s'", 
        ...
        compflag = TRUE;
#endif
	viewprintf(stderr, "placeq: Warning: filaments had identical centers.  This may correspond to a nonphysical overlap.\n");
	viewprintf(stderr, "  Make sure they are perpendicular.\n");
	viewprintf(stderr, "  center = (%g %g %g). Filaments are from seg = `%s' and seg `%s'", 
...
/* Determine side lengths. Make sure domain is eps bigger than problem. */
  ...
  if(length == 0) {
    viewprintf(stderr,"placeq: All the lengths in the problem are zero\n");
    FHExit(FH_NORMAL_END);
  }
  sys->length = length;
  sys->minx = minx;
  ...
  for(i = sys->depth - 1, side = sys->side/2; i >= 0; i--, side /= 2) {
	...
	for(l=0; l < side; l++) {
	  ...
	  if(nextc != NULL) {
		...
		viewprintf(stderr, "Couldn't find kid in parent's list. Exiting\n");
		FHExit(FH_GENERIC_ERROR);
	      }
	      /* remove from parent's list */
	      viewprintf(stderr, "Removing abandoned cube[%d][%d][%d][%d]\n",i,j,k,l);
...
int num_lexact, num_mexact;
  ...
  for(i=depth; i > 0; i--, side /= 2) {
    ...
    if (sys->debug == ON)
      viewprintf(stdout, "level %d, #mulexact %d, #locexact %d\n",i,num_mexact,num_lexact);
