...
SYS *indsys;
{
  ...
  if (precond_choice == ON || precond_choice == SPARSE) {
    ...
    if (indsys->opts->mat_vect_prod == DIRECT) 
      viewprintf(stderr,"Warning: -pposdef not supported with direct matrix\n \
...
double w;  /* frequency */
{
  ...
  for(i = 0; i < num_mesh; i++) {
    ...
    viewprintf(stderr,"Unknown Sparse Preconditioner\n");
...
double w;
{
  ...
  fillist = (Filcube *) DMALCORE(max_fils*sizeof(Filcube));	// Enrico, DMALCORE instead of malloc
  mat = (double **) DMALCORE(max_fils*sizeof(double *));		
  for(i = 0; i < max_fils; i++)
    mat[i] = (double *) DMALCORE(max_fils*sizeof(double));	
  ...
  for(seg = indsys->segment; seg != NULL; seg = seg->next) {
	...
	while(j < nsize && nc_pc[j] != pchg)
	  j++;
	if (j == nsize) {
	  viewprintf(stderr, "fill_bySegment: Hey, charge isn't in cube!\n");

	  DFREECORE(fillist);				// Enrico, added to free memory 
      for(i = 0; i < max_fils; i++)		// if exits beforehand
        DFREECORE(mat[i]);
	  DFREECORE(mat);
