...
SYS *indsys;
{
  ...
  if (indsys->opts->debug == ON)
    viewprintf(stdout, "To be broken ratio: %lg\n", rat);
  return rat;
}
...
ssystem *sys;
{
  ...
  viewprintf(stdout,"      cube    parent          Q2P              L2P               M2P\n"); 
  viewprintf(stdout," lvl  j,k,l    j,k,l   cubes size_mats    cubes size_mats   cubes size_mats\n"); 
	...
	for(k=0; k < side; k++) {
	  for(m = 0; m < side; m++) {
	    ...
	    if (nc != NULL) {
	      ...
	      if (na == NULL)
		na = nc;
	      viewprintf(stdout,"%3i  %3i %3i %3i   %3i %3i %3i  %3d %10d  %3d %10d  %3d %10d\n",
