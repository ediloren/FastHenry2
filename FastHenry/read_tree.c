...
void give_cell_adjaceny(Gcell *leaf, char leafdir, Gcell *nonleaf, char nonleafdir,
	G_nodes **pfareast_north, G_nodes **pfarwest_south);
...
void fprint_bi_kids(struct bi *two_kids, FILE *fp);
...
int process_plane(grndp, fp, indsys)
     GROUNDPLANE *grndp;
     FILE *fp;
     SYS *indsys;
{
  ...
  */
  ...
  for(contactp = gp->grndp->list_of_contacts; contactp != NULL && bFHContinue == TRUE; 
  ...
  if(bFHContinue == FALSE)  // Enrico, user stopped execution
	  return 1;
...
void get_nonuni_coords(x, y, z, gp, xr, yr, zr)
...
void get_global_coords(x, y, z, gp, xg, yg, zg)
...
void get_global_vec(x, y, z, gp, xg, yg, zg)
     double x,y,z, *xg, *yg, *zg;
     Nonuni_gp *gp;
{
...
Nonuni_gp *gp;
{
  ...
  if (retchar == NULL) {
    viewprintf(stderr, "Couldn't read first line\n");
    return 1;
  }
...
*/
  ...
  if (sscanf(line, "%d", &num_cells) != 1) {
    viewprintf(stderr, "First line should be an integer with total number of cells\n");
    return 1;
  }
  ...
  if (cells == NULL) {
    viewprintf(stderr, "Couldn't get space for %d cells\n",num_cells);
    return 1;
  }
  ...
  for(i = 0; i < num_cells; i++) {
    ...
    if (retchar == NULL) {
      viewprintf(stderr, "Unexpected end of file after %d lines\n", i+1);
      return 1;
    }
    ...
    if (numread != 5 && !(numread == 2 && strcmp(typename,"NONE") == 0)) {
      viewprintf(stderr, "At line %d: Couldn't read binary tree data\n",i+2);
      return 1;
    }
	...
	viewprintf(stderr, "Err at line %d: only binary trees allowed\n",i+2);
	return 1;
      }
      
      if (dum > num_cells || nchild1 > num_cells || nchild2 > num_cells) {
	viewprintf(stderr, "Err at line %d: index out of range\n",i+2);
	return 1;
      }
      
      if (cells[dum - 1].children != NULL) {
	viewprintf(stderr, "uh oh, this cell has been defined before: %d\n",dum);
      }
      
      if (cells[nchild1 - 1].parent != NULL)
	viewprintf(stderr, "uh oh, child %d for cell %d already has a parent\n",
		nchild1, dum);
	...
	viewprintf(stderr, "Unknown bintype at line %d\n", i+2);
	return 1;
      }
  ...
  if (cells[0].parent != NULL) {
    viewprintf(stderr, "uh oh, cells[0] has a parent but should be root cell!\n");
    return 1;
  }
...
void set_cell_coords(cell,x0,y0,x1,y1)
     Gcell *cell;
     double x0, y0, x1, y1;
{
  cell->x0 = x0;
...
void set_bi_coords( two_kids, x0, y0, x1, y1)
  Bi *two_kids;
  double x0,y0,x1,y1;
{
  if (two_kids->type == NS) {
...
void set_grid_coords(grid, x0, y0, x1, y1)
  Grid_2d *grid;
  double x0,y0,x1,y1;
{
  int i,j;
...
void resolve_bi_children(cell, info)
     Gcell *cell;
     Info *info;
{
  Bi *two_kids;
     ...
     int size;
{
  ...
  ptr = (void *) DCALCORE(1, size); // Enrico, DCALCORE instead of calloc;
									// cannot use CALCORE because it does not rely on
									// calloc so doesn't zero memory out
...
void Combine_edges(cell1, dir1, cell2, dir2)
     Gcell *cell1, *cell2;
     char dir1, dir2;
{
  G_edges *edge1, *edge2;
  ...
  if (is_leaf(cell1) || is_leaf(cell2))
    ...
    if (edge1->num_children != edge2->num_children) {
      viewprintf(stderr, "Bad discretization: number of children doesn't match %d != %d\n", 
...
void combine_node_info(cell1, dir1, cell2, dir2)
     Gcell *cell1, *cell2;
     char dir1, dir2;
{
...
void give_cell_adjaceny(leaf, leafdir, nonleaf, nonleafdir, 
...
void combine_nodes(leafcell, leafdir, fareast_north, farwest_south)
     Gcell *leafcell;
     char leafdir;
     G_nodes *fareast_north, *farwest_south;
{
...
/* replace the cell's node in the node_dir direction with new node
   */
void replace_node(old_node, new_node)
     G_nodes *new_node, *old_node;
{
  ...
  if (old_node->x != new_node->x || old_node->y != new_node->y)
   viewprintf(stderr, "replace_nodes: nodes don't have the same coords!\n");
#endif
...
{
#if GP_DEBUG == TRUE
 viewprintf(stderr, "killing node:");
...
void free_g_node(node)
     G_nodes *node;
{
  DFREECORE(node);
}

void determine_adjaceny(nodelist)
     G_nodes *nodelist;
{
  G_nodes *node;
     ...
     Nonuni_gp *gp;
{
  ...
  for(i = 1; i < num_z_pts - 1; i++) {
    ...
    viewprintf(stderr,"This isn't right, you didn't fix this\n");
...
void generate_segs(gp, indsys)
     Nonuni_gp *gp;
     SYS *indsys;
{
  G_nodes *node;
  //static complain = 0;  // Enrico, commented out 
  //static complain2 = 0; // because not used
  double x_shift, x_width, y_shift, y_width;
  
  gp->num_seg_groups = 0; 

  /* create segments in the north and east directions, and down(z),if needed */
  for(node = gp->nodelist; node != NULL && bFHContinue == TRUE; node = node->next) { // Enrico
    /* do north */
...
void get_width_and_shift(width_dir, node, leftcell, rightcell, ret_width, 
		     ret_shift)
     char width_dir;
     G_nodes *node;
...
void get_x_cell_vals(left, node, right, x_left, x_right)
     Gcell *left, *right;
     G_nodes *node;
...
void get_y_cell_vals(left, node, right, y_min, y_max)
     Gcell *left, *right;
     G_nodes *node;
     double *y_min, *y_max;
{
...
void make_segs(direction, node, othernode, width, x_shift, y_shift, gp, indsys)
     char direction;
...
{
  NODES *node0, *node1;
  // static double dontcare = 2.0; // Enrico, see header
  double *widthdir;
  char name[100];
  NODES *makenode();
  double xg,yg,zg;
...
void print_cell_and_kids(cell)
Gcell *cell;
{
  fprint_cell_and_kids(cell, stdout);
}

void fprint_cell_and_kids(cell, fp)
     Gcell *cell;
     FILE *fp;
{
  dump_cell(cell, fp);
...
void fprint_bi_kids(two_kids, fp)
     Bi *two_kids;
     FILE *fp;
{
...
void print_bi_addresses(two_kids, fp)
     Bi *two_kids;
     FILE *fp;
{
...
void dump_leaf_cells_to_file(cell, fname)
     Gcell *cell;
     char *fname;
{
  FILE *fp;
  ...
  if ( (fp = fopen(fname, "w")) == NULL) {
    viewprintf(stderr, "dump_leaf_cells_to_file: couldn't open file\n");
    return;
  }
