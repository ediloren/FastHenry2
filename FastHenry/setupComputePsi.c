...
// static CX *Ib = NULL, *Vb = NULL, *Vdirect = NULL;
...
#if OPCNT == ON
  viewprintf(stderr,"Inductance (mesh to branch) mults: %d\n",ind_opcnt_mult);
  viewprintf(stderr,"Just doing MRMtIm: %d\n",ind_opcnt_real);
   printops();
  ...
  if (maxdiff < .3)
    viewprintf(stderr,"maxdiff: %g  Vb[%d]=%g  Vdirect[%d]=%g\n",
...
double **mat;
{
  ...
  for(i = 0; i < numpchgs; i++) {
	...
	if (mat[i][j] != 0.0)
	  viewprintf(stderr,"Warning: dot product = %lg < EPS, but mat[i][j] = %lg\n",dotprod, mat[i][j]);
      }
    }
  }
}
